...
(function draw() {
    ...
    for (var id in players) {
        ...
        ctx.fill();
        ctx.restore();
        ctx.translate((sizes.back.size * sizes.scale * (base.x - player.x)) - (canvas.width / 2), (sizes.back.size * sizes.scale * (base.y - player.y)) - (canvas.height / 2));
...
socket.on("b", function (b) {
    ...
    for (var i = 0; i < b.length; ++i) {
        ...
        score.innerHTML = Math.round(b[i].Score);
        user.innerHTML = escapeHTML(b[i].Name);
        top.append(score,user);
    }
});
