...
var canvas=document.getElementById("map"),ctx=canvas.getContext("2d"),socket=io(),FPS={lastShow:0,lastCount:0,framesShown:0,calcFPS:!1,div:document.getElementById("fps")},sizes={back:{size:2E4,spacing:200,dotsize:3},ball:30,glow:0,scale:1},lastM=0,players={},view="#FFF",Base;function setCanvas(){canvas.width=$(document).width();canvas.height=$(document).height();ctx.font="bold "+Math.round(.5*sizes.ball*sizes.scale)+"px Rubik";ctx.textAlign="center"}(window.onresize=setCanvas)();
...
socket.on("u",function(a){players=a;a=document.getElementById("coord");(Base=players["/#"+socket.id])?a.innerHTML=String.fromCharCode(65+~~(9.999999999*Base.x))+" "+(~~(9.999999999*Base.y)+1):a.innerHTML=""});socket.on("si",function(a){var b=document.getElementById("si");document.getElementById("siN").innerHTML="Server: "+a.name;document.getElementById("siR").innerHTML="Region: "+a.region;b.style.height="auto";b.style.paddingBottom="5px"});
socket.on("b",function(a){var b=document.getElementById("score");Base&&Base.Health.val&&(b.style.height="20px",b.innerHTML=Math.round(Base.Score));for(var d=document.getElementById("leaders");d.firstChild;)d.removeChild(d.firstChild);for(var c=0;c<a.length;++c){var f=document.createElement("span"),e=document.createElement("span"),g=document.createTextNode(escapeHTML(a[c].Name));e.className="score";a[c].ID==="/#"+socket.id&&(e.className+=" your",f.className="your",b.style.height=0);e.appendChild(document.createTextNode(Math.round(a[c].Score)));
    d.appendChild(f).appendChild(e).parentNode.appendChild(g)}});socket.on("d",function(a){Base.Health=0;document.getElementById("output").innerHTML=(a.type?"You collided with ":"You were killed by ")+(escapeHTML(a.user).trim()||"an anonymous Wacher");openHome()});
